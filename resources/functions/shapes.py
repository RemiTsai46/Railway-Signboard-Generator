...
def circle(im:Image.Image, xy:Coords, radius:float, fill=None, outline=None,width=1):
    if fill == None and outline == None:
        ...
        circle = circle.resize((W,H),Image.Resampling.BOX)

        im.paste(color,(int(x0),int(y0)),circle)
        
def ellipse(im:Image.Image, xy:Coords, fill=None, outline=None,width=1):
    if fill == None and outline == None:
        pass
    else:
        #var init
